...
## The documentation was illustrated by dialog box screenshots

Producing major release documentation for the Abiquo product required capturing and cropping around 150 dialog box screenshots per release cycle. The dialog boxes appear bright against the darker background of the web application, so each screenshot had to be cropped to show only the dialog box before it could be published to the Abiquo wiki.
...
## Have you ever manually cropped 150 dialog box screenshots?

Manually cropping hundreds of screenshots for every major release was slow and tedious, requiring many hours of editing time. The process also had to handle several complicating factors:

- Screenshots could contain a dialog box **or** just a main screen, requiring a check before cropping  
...
I built a shell script using [Imagemagick](http://www.imagemagick.org/) to automate the cropping. The script works in two stages: first detecting whether a dialog box is present, then applying the appropriate crop command.
