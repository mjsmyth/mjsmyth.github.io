...
The API errors were included in the documentation on the Abiquo wiki.
...
I later also created a wiki page to describe the steps: [how-to create a tenant via API](https://mjsmyth.github.io/#/how_to_create_a_tenant_via_API).

## Python to implement API how-tos

When I write an API how-to, I think it's sometimes helpful to check the process with code.
I enjoyed using Python to quickly implement [How to add a NAT rule via API](https://abiquo.atlassian.net/wiki/spaces/doc/pages/311375630/How+to+add+a+NAT+rule+to+a+VM+via+API).
...
In some cases, the privileges required to use a request were not appearing in the API docs. Rather than check 100 pages containing 700 methods by hand, I wrote a script.
